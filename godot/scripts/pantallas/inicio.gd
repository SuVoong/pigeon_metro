...
const ITEM_X: float = 60.0
const ITEM_Y_START: float = 200.0
const ITEM_W: float = 200.0
const ITEM_H: float = 36.0
const ITEM_GAP: float = 46.0
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_up"):
		...
		var item: Dictionary = MENU_ITEMS[GameState.menu_cursor]
...
func _draw() -> void:
	...
	for i in MENU_ITEMS.size():
		...
		var selected: bool = i == GameState.menu_cursor
		var bg: Color = COLOR_ITEM_BG_SEL if selected else COLOR_ITEM_BG
		var border: Color = COLOR_TRAIN_YELLOW if selected else COLOR_CONCRETE
		var text_col: Color = COLOR_TRAIN_YELLOW if selected else COLOR_HUD

		draw_rect(Rect2(ITEM_X, y, ITEM_W, ITEM_H), bg)
		draw_rect(Rect2(ITEM_X, y, ITEM_W, ITEM_H), border, false, 1.0)
		draw_string(_font, Vector2(ITEM_X + 16, y + ITEM_H * 0.65),
			MENU_ITEMS[i].label, HORIZONTAL_ALIGNMENT_LEFT, -1, 14, text_col)

		if selected:
			_draw_arrow(46.0, y + ITEM_H / 2.0 - 3.0)
	...
	if int(_frame / 40) % 2 == 0:
		draw_string(_font, Vector2(60, h - 30),
			"UP/DOWN NAVEGAR  ENTER SELECCIONAR",
